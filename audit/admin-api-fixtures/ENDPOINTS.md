...
| --- | --- | --- | --- | --- | --- | --- |
| /v1/organization/usage/completions | GET | Aggregated token and cost buckets per organization/project | start_time, end_time, bucket_width, limit, page, group_by (project_id, user_id, api_key_id, model, batch), batch filter | `data[]` usage buckets, text/audio token counters, cached token fields, `has_more`, `next_page` | Org-level endpoints currently 60 req/min shared across admin surface | ✅ `usage_completions_fixture.json` |
